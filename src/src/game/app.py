...
class GameApp:
    ...
    def transition_to_victory(self, elapsed_time): #Called by PlayScreen when the game is won
        self.play_screen = None #Clear the play screen
        self.leaderboard.update(str(round(elapsed_time)))
    ...
    def transition_to_leaderboard(self): #Called by GameOverScreen when restart button is pressed
        self.start_screen = None #Clear the play screen
        self.leaderboard_screen = LeaderBoardScreen(self.screen, leaderboard=self.leaderboard.data) #Re-initialize the start screen
